...
def _build_unseen_reality_hook(article: Article, variant: int | None = None) -> str:
    ...
    if variant == 1 and data_points and len(sentences) > 1:
        ...
        # MORAL-STAKES: Collective failure / accountability framing.
        # Calm but damning — "we knew, we let it happen" energy.
        # Lead with a moral weight line, then context + cost.
        moral_openers = [
            "The warnings were there. They were ignored.",
            "This was preventable. It was allowed.",
            "We had the data. We made a choice.",
            "Someone signed off on this. Someone always does.",
            "The records show they knew. The records also show what they did next.",
        ]
        # Deterministic selection from title hash
        opener = moral_openers[sum(ord(c) for c in title) % len(moral_openers)]
        parts.append(opener)
        # Follow with the strongest concrete sentence for context
        if sentences:
            parts.append(sentences[0])

    elif variant == 4:
        # CONTRARIAN-REFRAME: "Everyone's looking at X, missing Y."
        # Redirect attention from the obvious angle to the deeper story.
        clean_title = title.rstrip(".!?").strip()
        # Build a "they're looking at X" + "the real story is Y" pair
        reframe_templates = [
            f"Everyone's talking about {clean_title.lower()}. They're missing the real story.",
            f"The headline says {clean_title.lower()}. Read past it.",
            f"You've heard the surface version. Here's what's underneath.",
            f"The loudest part of this story isn't the important part.",
        ]
        template = reframe_templates[sum(ord(c) for c in title) % len(reframe_templates)]
        parts.append(template)
        # Add a deeper-layer sentence
        if len(sentences) > 1:
...
def _refine_hook_with_exemplars(
    ...
    When exemplars are available and ``exemplars.inject_into_prompts`` is true,
    sends the raw hook + good_copy exemplars to Claude for a quality pass.
    Returns the original hook unchanged if exemplars aren't enabled, aren't
    available, or if the API call fails.
...
def _generate_hooks_llm(
    ...
    for pname, limit in platforms.items():
        conventions = {
            "twitter": "Punchy, self-contained. No link in body (link goes in reply). Max impact in short space.",
            "bluesky": "Slightly more room. Can be more atmospheric. Link card auto-generates.",
            "threads": "Most room. Can layer detail. End with a question to drive conversation.",
        }
...
def _build_post_text(
    ...
    it directly — skips the regex hook builder and exemplar refinement.
    Falls back to the template path when ``llm_hook`` is None.
    """
    if llm_hook:
        # LLM-generated hook — use directly, skip regex + exemplar refinement
        hook = llm_hook
    else:
        # Template path: regex extraction + optional exemplar refinement
        hook = _build_unseen_reality_hook(article)
        if not hook:
            # Quality gate rejected this hook (e.g. "CONTINUE READING" artifact).
            # Return empty — caller should skip this article.
            return ""
        # Exemplar-guided refinement: when good_copy exemplars exist and
        # inject_into_prompts is true, Claude sharpens the hook to match
        # flagged quality. Falls back silently to the template hook.
        hook = _refine_hook_with_exemplars(hook, article, platform, max_chars)
    ...
    # ── CTA rotation ──────────────────────────────────────────────
    # Cold-start mode: 50% no-link, 30% follow-driving, 20% link
    # Normal mode: 70% article, 20% subscribe, 10% no-link
    try:
    ...
    if _is_cold:
        # Cold-start rotation: maximize algorithmic distribution
        if rotation_slot in (0, 1, 2, 3, 4):
            # 50%: no link — pure engagement play
            link = ""
            tag_str = ""
        elif rotation_slot in (5, 6, 7):
            # 30%: follow-driving — hook ends with follow prompt
            link = ""
            tag_str = ""
            # Append a follow nudge to the hook
            _follow_prompts = [
                "Follow @docketclimate for more.",
            ...
            # 20%: article/subscribe link (preserved for some clicks)
            if rotation_slot == 9:
                link = SUBSCRIBE_URL
        ...
        if rotation_slot in (3, 7):
            ...
            # 10%: no link at all — pure engagement play
            link = ""
            tag_str = ""
    ...
    if platform == "twitter":
        ...
        if link:
            ...
            # Pure engagement: conversation hook only, no link
            suffix = f"\n\n{convo_hook}"

        available = max_chars - len(suffix)
        if available < 60 and link:
            # Drop tags + conversation hook to fit
            suffix = f"\n\n{link}"
            available = max_chars - len(suffix)
        ...
        # Bluesky and other platforms: hook + link + hashtags
        suffix_parts = []
        if link:
        ...
        truncated = _truncate_to_limit(hook, available)
        return f"{truncated}{suffix_with_gap}"
...
def _format_exemplars_for_prompt(
    ...
    lines.append(
        "Study these for tone, concreteness, data placement, and word economy. "
        "Match their quality."
    )
    return "\n".join(lines)
...
def generate_video_scripts(issue: DocketIssue, config: dict) -> list[VideoScript]:
    ...
    for script_idx, article in enumerate(best):
        ...
        if not hook:
            console.print(
                f"  [yellow]Skipping video for {article.title[:50]} "
                f"(hook quality gate)[/yellow]"
            )
            continue
...
def generate_all_video_scripts(issue: DocketIssue, config: dict) -> list[VideoScript]:
    ...
    for script_idx, article in enumerate(best):
        ...
        if not hook:
            console.print(
                f"  [yellow]Skipping video draft for {article.title[:50]} "
                f"(hook quality gate)[/yellow]"
            )
            continue
